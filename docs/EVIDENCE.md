...
spawned by `examples/harness/`. No signal was sent to any unrelated process; every run used
`--audit-only --no-llm`. Reproduce with:
...
`os.path.realpath(path)`, not the path string you started with. `examples/harness/run_audit_workload.py::_render_scope` does this; `examples/harness/scope.audit-demo.yaml` documents it
...
`examples/harness/run_audit_workload.py` on your own target shape and poll interval before
trusting a CPU budget for it.
